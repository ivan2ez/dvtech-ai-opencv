...
/** GET /api/reports/archived — admin only. Lists archived reports. */
export async function listArchivedReportsHandler(
  req: AuthenticatedRequest,
  res: Response,
  next: NextFunction
): Promise<void> {
  try {
    ...
    const data = await listArchivedRecords(Report);
...
/** DELETE /api/reports/:id/archive — admin only. Archive (soft delete). */
export async function archiveReportHandler(
  req: AuthenticatedRequest,
  res: Response,
  next: NextFunction
): Promise<void> {
  try {
    ...
    const id = parseInt(req.params.id as string, 10);
    await archiveRecord(Report, id, 'Report');
    res.status(200).json({ message: 'Report moved to the archive' });
  } catch (error) {
    next(error);
  }
}

/** POST /api/reports/:id/restore — admin only. Restore an archived report. */
export async function restoreReportHandler(
  req: AuthenticatedRequest,
  res: Response,
  next: NextFunction
): Promise<void> {
  try {
    ...
    const id = parseInt(req.params.id as string, 10);
    await restoreRecord(Report, id, 'Report');
...
/** DELETE /api/reports/:id/permanent — admin only. Permanently delete. */
export async function deleteReportPermanentlyHandler(
  req: AuthenticatedRequest,
  res: Response,
  next: NextFunction
): Promise<void> {
  try {
    ...
    const id = parseInt(req.params.id as string, 10);
    await permanentlyDeleteRecord(Report, id, 'Report');
