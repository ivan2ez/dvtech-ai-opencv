...
function getNavLinks(role: UserRole | null): NavLink[] {
  ...
  switch (role) {
    case 'customer':
      // Customers have no dashboard (removed per revisions) — send them to
      // their service requests instead.
      return [
        { label: 'Home', to: '/' },
        { label: 'Services', to: '/services' },
        { label: 'Products', to: '/products' },
    ...
    <header className="sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="container mx-auto flex h-16 items-center justify-between px-4">
        ...
        <Link to="/" className="flex items-center gap-2 font-bold text-lg">
          ...
          <span className="text-foreground">DVTech</span>
        </Link>
            ...
            <SheetHeader>
              <SheetTitle className="text-left">
                <Link to="/" className="flex items-center gap-2" onClick={() => setMobileOpen(false)}>
                  ...
                  <span className="font-bold text-foreground">DVTech</span>
                </Link>
              </SheetTitle>
            </SheetHeader>
      ...
      <ConfirmDialog
        ...
        title="Sign out?"
        description="You will be returned to the home page and will need to sign in again to access your account."
