...
/**
 * Horsepower, BTU capacity and price are intentionally uncapped so the catalog
 * can carry commercial units. The only rejections are non-numeric input
 * (letters) and negative numbers.
 */
const productSchema = z.object({
  brand: z.string().trim().min(1, 'Brand is required').max(100, 'Brand must be 100 characters or less'),
  model: z.string().trim().min(1, 'Model is required').max(100, 'Model must be 100 characters or less'),
  type: z.enum(['split-type', 'window-type', 'floor-standing'], {
    error: 'Please select a valid type',
  }),
  unitType: z.enum(['inverter', 'non-inverter'], {
    error: 'Please choose a unit type',
  }),
  horsepower: z.coerce
    .number({ error: 'Horsepower must be a number' })
    .nonnegative('Horsepower cannot be negative')
    .gt(0, 'Horsepower must be greater than 0'),
  btuCapacity: z.coerce
    .number({ error: 'BTU capacity must be a number' })
    .int('BTU capacity must be a whole number')
    .nonnegative('BTU capacity cannot be negative')
    .gt(0, 'BTU capacity must be greater than 0'),
  price: z.coerce
    .number({ error: 'Price must be a number' })
    .nonnegative('Price cannot be negative')
    .gt(0, 'Price must be greater than 0'),
  // Higher weight shows the product earlier on the public catalog (a proxy for
  // best-selling). Whole number, 0 or more; 0 means "no boost".
  sortWeight: z.coerce
...
function getUnitTypeLabel(unitType: string): string {
  return UNIT_TYPE_OPTIONS.find((u) => u.value === unitType)?.label ?? unitType;
}
...
export function ManageProducts() {
  ...
  // Active catalog vs Archive (recycle bin) view.
  const [activeTab, setActiveTab] = useState<'active' | 'archive'>('active');
  // Archive an active product to the recycle bin.
  const [archivingProduct, setArchivingProduct] = useState<AirconProduct | null>(null);
  const [isArchiving, setIsArchiving] = useState(false);
  // Bumped after archiving so the ArchivePanel refetches.
  const [archiveRefreshKey, setArchiveRefreshKey] = useState(0);

  // Details dialog state
  const [detailProduct, setDetailProduct] = useState<AirconProduct | null>(null);
  const [detailDialogOpen, setDetailDialogOpen] = useState(false);

  function handleOpenDetails(product: AirconProduct) {
  ...
  function handleOpenEdit(product: AirconProduct) {
    ...
    form.reset({
      ...
      model: product.model,
      type: product.type,
      unitType: product.unitType ?? 'non-inverter',
  ...
  async function handleConfirmDelete() {
    ...
    const label = `${deletingProduct.brand} ${deletingProduct.model}`;
    try {
  ...
  /** Archives an active product to the recycle bin (separate from deactivate). */
  async function handleConfirmArchive() {
    if (!archivingProduct) return;
    const label = `${archivingProduct.brand} ${archivingProduct.model}`;
    setIsArchiving(true);
    try {
      ...
      toast.error('Failed to archive the product. Please try again.');
    } finally {
    ...
    try {
      ...
      toast.success(
        isEditing
          ...
          : `${values.brand} ${values.model} created.`
      );
    } catch (err) {
    ...
    <div className="p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Add, edit, and manage AC products in the catalog</p>
        </div>
          ...
          <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                />
                {/* Unit type — inverter vs non-inverter drives the AI's
                    running-cost advice, so it's captured explicitly. */}
                <FormField
                  control={form.control}
                  name="unitType"
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <div role="radiogroup" aria-label="Unit type" className="flex gap-4 pt-1">
                ...
                />
                {/* No upper limits on HP, BTU or price — only letters and
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <p className="text-xs text-muted-foreground">
                        Higher numbers appear first on the public catalog (use it to promote
                        best-sellers). 0 = normal.
                      </p>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                />
                <p className="text-xs text-muted-foreground">
                  Add product photos using the image manager (image icon) on the product row after saving.
                </p>
      ...
      <div className="flex items-center gap-1 border-b">
        {(['active', 'archive'] as const).map((tab) => (
          ...
          >
            {tab === 'active' ? 'Products' : 'Archive'}
          </button>
        ))}
      </div>

      {activeTab === 'archive' && (
        <ArchivePanel<AirconProduct>
          itemLabel="product"
          fetchArchived={getArchivedProducts}
          onRestore={restoreProduct}
          onPermanentDelete={deleteProductPermanently}
          refreshKey={archiveRefreshKey}
          columns={[
            { header: 'Brand', render: (p) => p.brand },
            { header: 'Model', render: (p) => p.model },
            { header: 'Type', render: (p) => getTypeLabel(p.type) },
            { header: 'HP', render: (p) => p.horsepower },
                ...
                {filteredProducts.length === 0 ? (
                  ...
                  filteredProducts.map((product) => (
                    ...
                    >
                      ...
                      <TableCell>
                        <Badge variant="outline">{getUnitTypeLabel(product.unitType)}</Badge>
                      </TableCell>
                      ...
                      <TableCell onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center gap-1">
                          ...
                          <Button
                            ...
                            aria-label={`Archive ${product.brand} ${product.model}`}
                ...
                >
                  <CardContent className="p-4 space-y-3">
                    ...
                    <div className="space-y-1 text-sm">
                      <p><span className="text-muted-foreground">Type:</span> {getTypeLabel(product.type)} • {getUnitTypeLabel(product.unitType)}</p>
      ...
      <DetailDialog
        ...
        title={detailProduct ? `${detailProduct.brand} ${detailProduct.model}` : ''}
        subtitle={detailProduct ? getTypeLabel(detailProduct.type) : undefined}
        headerRight={
          detailProduct ? (
            <Badge variant={detailProduct.isActive ? 'default' : 'secondary'}>
        ...
        {detailProduct && (
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <DetailItem icon={TagIcon} label="Brand" value={detailProduct.brand} />
              <DetailItem icon={BoxIcon} label="Type" value={getTypeLabel(detailProduct.type)} />
              <DetailItem
                icon={ZapIcon}
                label="Unit Type"
                value={getUnitTypeLabel(detailProduct.unitType)}
              />
              <DetailItem
                icon={ZapIcon}
                label="Capacity"
                value={`${detailProduct.horsepower} HP • ${detailProduct.btuCapacity.toLocaleString()} BTU`}
              />
              <DetailItem
                icon={DollarSignIcon}
                label="Price"
                value={formatPrice(detailProduct.price)}
              />
            </div>
      ...
      <Dialog open={archivingProduct !== null} onOpenChange={(open) => !open && setArchivingProduct(null)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
          ...
          <p className="text-sm text-muted-foreground">
            Move{' '}
            <span className="font-medium text-foreground">
              {archivingProduct?.brand} {archivingProduct?.model}
            </span>{' '}
            to the archive? You can restore it or delete it permanently from the Archive tab.
          </p>
          <DialogFooter>
            <Button variant="outline" onClick={() => setArchivingProduct(null)} disabled={isArchiving}>
              Cancel
            </Button>
