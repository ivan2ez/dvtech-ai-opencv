...
export function Reports() {
  ...
  // Report history vs Archive view.
  const [showArchive, setShowArchive] = useState(false);
  const [archiveRefreshKey, setArchiveRefreshKey] = useState(0);

  /** Archives a report to the recycle bin. */
  async function handleArchiveReport(report: ReportRecord) {
    try {
      await archiveReport(report.id);
      await fetchReportHistory(pagination.page);
      setArchiveRefreshKey((k) => k + 1);
      toast.success(`Report #${report.id} moved to the archive.`);
    } catch (err) {
      console.error('Failed to archive report:', err);
      toast.error('Failed to archive the report.');
    }
  }

  function handleOpenDetails(report: ReportRecord) {
    ...
    <div className="p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Generate and view system performance reports</p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle>{showArchive ? 'Archived Reports' : 'Report History'}</CardTitle>
            <div className="flex items-center gap-2">
              <Button
              ...
              >
                ...
                {showArchive ? 'Back to History' : 'Archive'}
              </Button>
              {!showArchive && (
                <Button
                  variant="outline"
        ...
        <CardContent>
          {showArchive && (
            <ArchivePanel<ReportRecord>
              itemLabel="report"
              fetchArchived={getArchivedReports}
              onRestore={restoreReport}
              onPermanentDelete={deleteReportPermanently}
              refreshKey={archiveRefreshKey}
              columns={[
                { header: 'ID', render: (r) => `#${r.id}` },
                { header: 'Type', render: (r) => REPORT_TYPE_LABELS[r.reportType as ReportType] || r.reportType },
                { header: 'Summary', render: (r) => r.summary || '—', className: 'max-w-[280px] truncate' },
                { header: 'Generated', render: (r) => formatDate(r.generatedDate) },
              ]}
            />
          )}

          {!showArchive && historyError && (
          ...
          {!showArchive && isLoadingHistory && !historyError && (
                          ...
                          <TableCell onClick={(e) => e.stopPropagation()}>
                            <div className="flex items-center gap-1">
                              ...
                              <Button
                                variant="ghost"
                                size="sm"
                                onClick={() => void handleArchiveReport(report)}
                                aria-label={`Archive report #${report.id}`}
              ...
              <div className="md:hidden space-y-3">
                {reports.length === 0 ? (
                  ...
                  sortedReports.map((report) => (
                    <div
                      key={report.id}
                      className="rounded-md border p-4 space-y-3 cursor-pointer hover:bg-muted/50 transition-colors"
                      onClick={() => handleOpenDetails(report)}
                    >
                      <div className="flex items-center justify-between">
      ...
      <DetailDialog
        ...
        title={detailReport ? `Report #${detailReport.id}` : ''}
        subtitle={detailReport ? `Generated ${formatDate(detailReport.generatedDate)}` : undefined}
        headerRight={
          detailReport ? (
            <Badge variant="outline">
              {REPORT_TYPE_LABELS[detailReport.reportType as ReportType] || detailReport.reportType}
            </Badge>
          ) : undefined
        }
        actions={
          detailReport ? (
            <>
              <Button
                variant="outline"
                size="sm"
                disabled={exportingId === detailReport.id}
                onClick={() => void handleExport(detailReport.id, 'csv')}
              >
                <DownloadIcon className="h-4 w-4 mr-1" />
                CSV
              </Button>
              <Button
                ...
                size="sm"
                disabled={exportingId === detailReport.id}
                onClick={() => void handleExport(detailReport.id, 'pdf')}
              >
                <DownloadIcon className="h-4 w-4 mr-1" />
                PDF
              </Button>
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <DetailItem
                icon={TagIcon}
                label="Report Type"
                value={REPORT_TYPE_LABELS[detailReport.reportType as ReportType] || detailReport.reportType}
              />
              <DetailItem
                icon={CalendarClockIcon}
                label="Generated"
                value={formatDate(detailReport.generatedDate)}
              />
              {detailReport.serviceRequestId != null && (
                <DetailItem
                  icon={ClipboardListIcon}
                  label="Service Request"
                  value={`#${detailReport.serviceRequestId}`}
                />
              )}
            </div>

            <DetailTextBlock icon={FileTextIcon} label="Summary">
