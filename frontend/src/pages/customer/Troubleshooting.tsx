...
const ISSUE_MIN = 10;
const ISSUE_MAX = 2000;
const BRAND_MODEL_MAX = 100;
...
/**
 * Maps each common symptom to the service the request form should default to,
 * per the revisions table. Most faults are Repair; odor and weak airflow are
 * typically resolved by Cleaning.
 */
const SYMPTOM_TO_SERVICE: Record<string, string> = {
  ...
  'Bad smell': 'Cleaning',
  "AC won't turn on": 'Repair',
  'Weak airflow': 'Cleaning',
  'Ice forming on unit': 'Repair',
  'Remote not working': 'Repair',
};

/**
 * Picks the service to prefill from the selected symptoms. If any selected
 * symptom maps to Repair, Repair wins (it's the safer default for a fault);
 * otherwise use the first mapped service (e.g. Cleaning). Defaults to Repair.
 */
function serviceForSymptoms(symptoms: string[]): string {
  const mapped = symptoms.map((s) => SYMPTOM_TO_SERVICE[s]).filter(Boolean);
  if (mapped.length === 0) return 'Repair';
  if (mapped.includes('Repair')) return 'Repair';
  return mapped[0] ?? 'Repair';
}
...
export function Troubleshooting() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (trimmedIssue.length < ISSUE_MIN) {
      setIssueError(`Please describe the issue in at least ${ISSUE_MIN} characters.`);
      return;
    }
    setIssueError('');
    // Don't submit while an image validation error is showing.
    if (imageError) {
      return;
    }
  ...
  /**
   * Opens the in-place request-service modal with the service preselected based
   * on the symptoms the customer chose (e.g. "Water leaking" -> Repair).
   */
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            ...
            and suggest fixes you can try — or book a technician right away.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {errorMessage && (
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center justify-between">
                {issueError ? (
                  <p className="text-xs font-medium text-destructive">{issueError}</p>
                ) : (
                  <span />
                )}
                <p className="text-xs text-muted-foreground">
                  {issue.trim().length}/{ISSUE_MAX} (minimum {ISSUE_MIN})
                </p>
              </div>
            </div>
      ...
      {result && (
        <TroubleshootingResultCard
          result={result}
          suggestedService={serviceForSymptoms(selectedSymptoms)}
          onRequestService={requestService}
        />
      )}

      {/* In-place request-service modal, launched from the result card. */}
      <RequestServiceModal
        open={requestServiceOpen}
        onOpenChange={setRequestServiceOpen}
        suggestedService={serviceForSymptoms(selectedSymptoms)}
        initialDetails={issue}
      />
    </div>
  );
}

function TroubleshootingResultCard({
  result,
  suggestedService,
  onRequestService,
}: {
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              >
                ...
                Request Service ({suggestedService})
              </Button>
            </div>
          </div>
        </div>
